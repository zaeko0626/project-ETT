...
// ---------------- Orders grid CSS (inline helpers remain) ----------------
function ensureRequestsGridCSS() {
  ...
  st.textContent = `
    ...
    .hdr-icon:hover{ background: rgba(255,255,255,.06); }
...
function renderKpis(){
  ...
  const card = (title, value, chip, clickStatus) => `
    <div class="kpi-card" onclick="${clickStatus ? `applyKpiStatus('${esc(clickStatus)}')` : `clearHeaderFilter('status')`}">
      <div class="kpi-title">${esc(title)}</div>
      <div class="kpi-value">${esc(value)}</div>
      <div class="kpi-chip">${esc(chip)}</div>
    </div>
  `;
    ...
    if (isAdmin()) {
      ...
      return `
        <div class="request-row" style="grid-template-columns:${$("requests-header")?.style.gridTemplateColumns || ""};" onclick="openRequestDetail('${esc(r.request_id)}')">
          ...
          <div><span class="${esc(st.cls)} status">${esc(st.label)}</span></div>
...
window.openRequestDetail = (request_id) => {
  ...
  const tableHead = `
    <div style="padding:0 12px 12px;">
      <div style="display:grid; grid-template-columns: 2.2fr 1.4fr 0.9fr 1.2fr 1.6fr; gap:12px; font-weight:900; opacity:.85; padding:10px 0; border-bottom:1px solid rgba(255,255,255,.08);">
  ...
  const bodyRows = lines.map((line) => {
    ...
    return `
      <div style="display:grid; grid-template-columns: 2.2fr 1.4fr 0.9fr 1.2fr 1.6fr; gap:12px; padding:10px 0; border-bottom:1px solid rgba(255,255,255,.06); align-items:center;">
        <div>${item}</div>
        <div>Размер: ${size}</div>
        <div>${qty} ширхэг</div>
        <div><span class="${esc(meta.cls)} status">${esc(meta.label)}</span></div>
...
async function renderUserHistory() {
  ...
  try {
    ...
    box.innerHTML = `<div style="padding:12px; color:#ff6b6b;">${esc(e.message || String(e))}</div>`;
  }
}
